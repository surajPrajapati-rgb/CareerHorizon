...
def test_message_detail_get(api_client):
    ...
    response = api_client.get(f'messaging/message/{message.id}/')
...
def test_message_detail_update(api_client):
    ...
    response = api_client.put(f'messaging/message/{message.id}/', updated_content, format='json')
    assert response.status_code == status.HTTP_200_OK
...
def test_message_detail_delete(api_client):
    ...
    # Test: Delete message
    response = api_client.delete(f'messaging/message/{message.id}/')
    assert response.status_code == status.HTTP_200_OK
