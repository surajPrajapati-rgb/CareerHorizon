...
def test_chat_users(api_client, create_users, create_messages):
    ...
    response = api_client.get('http://localhost:8000/messaging/chat_users/', {'sender': user1.username})
...
def test_chat_users_no_communication(api_client, create_users):
    ...
    response = api_client.get('http://localhost:8000/messaging/chat_users/', {'sender': user3.username})
