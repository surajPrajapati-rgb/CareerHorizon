...
def test_chat_history(api_client, create_users, create_messages):
    ...
    response = api_client.get(f'http://localhost:8000/messaging/chat/{user2.username}/', {'sender': user1.username})
...
def test_chat_history_user_not_found(api_client, create_users):
    ...
    response = api_client.get('http://localhost:8000/messaging/chat/nonexistent_user/', {'sender': user1.username})
...
def test_chat_history_missing_sender(api_client, create_users):
    ...
    response = api_client.get(f'http://localhost:8000/messaging/chat/{user2.username}/')
